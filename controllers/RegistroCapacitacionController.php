...
class RegistroCapacitacionController {
    ...
    private function mapearDatosFormulario() {
        ...
        // 🔥 CAMBIADO DE CURSO A PROGRAMA
        $registro->setProgramaId($_POST['programa_id']);
    ...
    public function buscarPorDni() {
        ...
        if (!empty($dni)) {
            ...
            $resultado = $this->model->buscarPorDni($dni);

            $capacitaciones = $resultado ? $resultado : [];
        }

        $pagina_actual = 'registros_capacitacion';

        // 🔥 CAMBIADO DE CURSOS A PROGRAMAS
        $modelPrograma = new ProgramaEducativoModel();
        $programas = $modelPrograma->cargar();
