                            ...
                            <tbody>
                                ...
                                if (!empty($cursos)):
                                    foreach ($cursos as $curso): 
                                        // Obtenemos valores con getters
                                        $tipo = $curso->getTipo() ?? '';
                                        $codigo = $curso->getCodigoCurso() ?? '';
                                        $nombre = $curso->getNombreCurso() ?? '';
                                        $horas = $curso->getHorasTotales() ?? '';

                                        // Estilos dinámicos
                                        $badgeClass = ($tipo === 'diplomados') ? 'badge-diplomado' : 'badge-certificado';
                                        $tipoFormateado = ucfirst(substr($tipo, 0, -1));
                                ?>
                                <tr class="fila-curso">
                                    <td class="id-column"><?= $i++ ?></td>
                                    <td><span class="codigo-box"><?= htmlspecialchars($codigo) ?></span></td>
                                    <td><strong><?= htmlspecialchars($nombre) ?></strong></td>
                                    <td><span class="badge <?= $badgeClass ?>"><?= htmlspecialchars($tipoFormateado) ?></span></td>
                                    <td><i class="far fa-clock" style="color: #94a3b8; margin-right: 5px;"></i> <?= htmlspecialchars($horas) ?> h</td>
