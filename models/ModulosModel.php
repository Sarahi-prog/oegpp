...
class ModulosModel {
    ...
    public function buscarPorPrograma($id_programa) {
        $sql = "SELECT id_modulo, programa_id, nombre_modulo, horas, fecha_inicio, fecha_fin, estado FROM modulos WHERE curso_id = :id";
        $ps = $this->db->prepare($sql);
        $ps->bindValue(':id', $id_programa, PDO::PARAM_INT);
        $ps->execute();
        return $this->mapearModulos($ps->fetchAll(PDO::FETCH_ASSOC));
    }

    public function cargar() {
        $sql = "SELECT id_modulo, programa_id, nombre_modulo, horas, fecha_inicio, fecha_fin, estado FROM modulos";
        $ps = $this->db->prepare($sql);
        $ps->execute();
        return $this->mapearModulos($ps->fetchAll(PDO::FETCH_ASSOC));
    }
    ...
    private function mapearModulos($filas) {
        ...
        foreach($filas as $f) {
            ...
            $mod->setProgramaId($f['programa_id']);
    ...
    public function guardar(Modulos $modulo) {
        $sql = "INSERT INTO modulos (programa_id, nombre_modulo, horas, fecha_inicio, fecha_fin, estado) 
                VALUES (:cid, :nm, :h, :fi, :ff, 1)";
        $ps = $this->db->prepare($sql);
        $ps->bindValue(":cid", $modulo->getProgramaId());
    ...
    public function modificar(Modulos $modulo) {
        $sql = "UPDATE modulos SET programa_id = :cid, nombre_modulo = :nm, horas = :h, 
                fecha_inicio = :fi, fecha_fin = :ff WHERE programa_id = :id";
        $ps = $this->db->prepare($sql);
        $ps->bindValue(":cid", $modulo->getProgramaId());
        $ps->bindValue(":nm", $modulo->getNombreModulo());
        $ps->bindValue(":h", $modulo->getHoras());
