...
class AdministradoresModel {
    ...
    public function guardarTokenSesion($id_admin, $token) {
    $sql = "UPDATE administradores SET token_sesion = :token WHERE id_admin = :id";
    $ps = $this->db->prepare($sql);
...
public function verificarTokenSesion($id_admin, $token) {
    $sql = "SELECT token_sesion FROM administradores WHERE id_admin = :id";
    $ps = $this->db->prepare($sql);
    $ps->bindParam(":id", $id_admin);
    $ps->execute();
    $fila = $ps->fetch(PDO::FETCH_ASSOC);
    return $fila && $fila['token_sesion'] === $token;
}

public function obtenerTodos() {
    $sql = "SELECT id_admin, usuario, correo, rol FROM administradores WHERE verificado = true ORDER BY usuario";
    $ps = $this->db->prepare($sql);
    $ps->execute();
    return $ps->fetchAll(PDO::FETCH_ASSOC);
}

public function resetearPassword($id_admin, $passwordHash) {
    $sql = "UPDATE administradores SET password = :p WHERE id_admin = :id";
    $ps = $this->db->prepare($sql);
    $ps->bindParam(":p", $passwordHash);
    $ps->bindParam(":id", $id_admin);
    return $ps->execute();
}
}
