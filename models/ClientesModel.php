...
class ClientesModel {
    ...
    public function obtenerClientesPaginados($pagina = 1, $registrosPorPagina = 20) {
        $offset = ($pagina - 1) * $registrosPorPagina;

        $query = "SELECT id_cliente, dni, nombres, apellidos, correo, celular, area, estado 
                FROM clientes 
    ...
    // CAMBIO AQUÍ: Usar $this->conexion en lugar de $this->db
    $ps = $this->conexion->prepare($sql); 
    
    $ps->execute();
    $filas = $ps->fetchAll(); // Corregido: fetchall -> fetchAll (es sensible a mayúsculas en algunas versiones)
    $clientes = array();
